# BiliBilişim
BUTGEM Yazılım Uzmanlığı Bitirme Projesi
Bu Proje Bursa Sanayi Ve Ticaret Odası Eğitim Vakfı(BUTGEM) Yazılım Uzmanlığı Kursu bitirme projesidir. Entity Framework kullanılarak MVC yöntemiyle veritabanı entegrasyonlu bir E-Ticaret sitesi yapılmıştır. 

Proje MSSQL veritabanında localde çalışmaktadır ve veritabanı scripti proje dosyaları arasında bulunmaktadır.Kolaylık sağlanması için ekte admin ve vip/vip olmayan kullanıcı bilgileri verilmiştir:

* Admin Kullanıcı Adı: Butgem
* Admin Şifresi: 123456

* VIP Kullanıcı Adı: fadime231
* VIP Şifre: 987654321

* Diğer Kullanıcı Adı: mahmut
* Şifre: 12345678


This project is the final project for the Bursa Chamber of Commerce and Industry Education Foundation (BUTGEM) Software Expertise Course. An E-Commerce website with database integration was developed using the MVC pattern and Entity Framework.

The project runs locally on an MSSQL database, and the database script is included among the project files. For convenience, admin, VIP, and non-VIP user credentials are provided below:

* Admin Username: Butgem
* Admin Password: 123456

* VIP Username: fadime231
* VIP Password: 987654321

* Other Username: mahmut
* Password: 12345678